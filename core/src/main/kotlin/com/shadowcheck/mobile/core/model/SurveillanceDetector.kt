...
class SurveillanceDetector @Inject constructor() {
    ...
    fun detectThreats(
        wifiNetworks: List<com.shadowcheck.mobile.domain.model.WifiNetwork>,
        btDevices: List<com.shadowcheck.mobile.domain.model.BluetoothDevice>
    ): List<ThreatDetection> {
        val threats = mutableListOf<ThreatDetection>()
        
        // Detect rogue APs
        wifiNetworks.filter { it.ssid.contains("Free", ignoreCase = true) || it.ssid.isBlank() }.forEach {
            threats.add(ThreatDetection(
                type = ThreatType.ROGUE_AP,
                severity = ThreatSeverity.HIGH,
                description = "Suspicious network: ${it.ssid}",
