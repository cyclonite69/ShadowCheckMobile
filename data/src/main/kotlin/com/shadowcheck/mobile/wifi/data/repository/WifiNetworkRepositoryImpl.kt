...
import com.shadowcheck.mobile.wifi.data.local.entity.toEntity
...
import com.shadowcheck.mobile.wifi.data.remote.dto.toEntity
...
class WifiNetworkRepositoryImpl @Inject constructor(
    ...
    override fun getNetworkBySsid(ssid: String): Flow<WifiNetwork?> {
    ...
    override suspend fun syncWithWiGLE(apiKey: String): Int = withContext(dispatcher) {
        try {
            ...
            if (response.isSuccessful) {
                val entities = response.body()?.results?.map { it.toEntity() }.orEmpty()
